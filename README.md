...
- 建立统一的平台目录和接入矩阵，按 CSGOSKINS.GG markets 目录纳入 32 个市场，并补充 market.csgo.com 与 Google 检索确认的主流市场。
- 将当前样例交易标的限定为武器箱、贴纸、印花胶囊和签名胶囊，不再保留皮肤类样例。
...
| 平台覆盖 | 覆盖 45 个市场，包括 32 个 CSGOSKINS.GG markets 目录市场、market.csgo.com 和 12 个补充市场 |
| 样例行情覆盖 | 为 45 个平台生成确定性样例盘口，参与报价矩阵和利差排序 |
| 标的范围 | 当前只保留武器箱、贴纸、印花胶囊和签名胶囊；武器箱和胶囊分类按来源分类页全量纳入 |
...
平台列表以 CSGOSKINS.GG markets 目录为基础，并补充 market.csgo.com 与 Google 检索确认的主流市场。当前已纳入 45 个市场：

Steam、CSFloat、LIS-SKINS、CS.MONEY、Skinport、Tradeit.gg、BUFF163、GameBoost、DMarket、CS2、PirateSwap、SkinSwap、Skin.Land、Avan.market、Skinflow、Aim.market、Mannco.store、white.market、SkinBaron、SkinPlace、BUFF Market、ShadowPay、WAXPEER、SkinOut、CS.TRADE、Skinvault、UUSKINS、Skins.com、CSGO Market、Swap.gg、LOOT.Farm、Skins.Cash、SkinCashier、Skinwallet、CS.Deals、iTrade.gg、Clash.gg Marketplace、SkinsMonkey、C5Game、YouPin、IGXE、HaloSkins、Exeskins、NTSkins、Dupe。

说明：纳入目录和样例行情覆盖不等于已完成真实交易接入。当前系统用目录级样例盘口完成 45 平台报价矩阵和利差排序，同时明确标记真实行情、费率、库存、余额、执行和对账能力的正式接入状态。

## 标的范围

当前样例交易标的不再包含武器皮肤，只包含以下类别：

| 类别 | 数量 | 说明 |
| --- | ---: | --- |
| 武器箱 | 45 | CSGOSKINS.GG weapon-case 分类项 |
| 印花胶囊 | 48 | CSGOSKINS.GG sticker-capsule 分类项 |
| 签名胶囊 | 48 | CSGOSKINS.GG autograph-capsule 分类项 |
| 贴纸 | 48 | CSGOSKINS.GG sticker 分类样例 |
...
| `docs/平台接入矩阵.md` | 45 个市场的目录字段、能力字段和接入状态 |
