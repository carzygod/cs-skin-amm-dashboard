...
- 提供一个可运行的利差机会中心，展示机会排序、费用拆解、流动性检查、报价矩阵、平台接入状态和交易所连通性。
...
| 净利润计算 | 扣除卖出费用、买入费用、资金成本、汇率成本和风险缓冲 |
...
说明：纳入目录不等于已完成真实交易接入。当前系统会明确标记真实行情、费率、库存、余额、执行和对账能力的接入状态，并在交易所列表中显示平台官网或目录页的连通性与延迟。
